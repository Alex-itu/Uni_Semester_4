...
(* Question 1.2: Negation and subtraction *)
    ...
    let rec subtract a b = 
        match a, b with
        | Num i1, Num i2 -> Add (Num i1, Num -i2)
        | add1 , add2 ->  Add (add1, negate add2)
...
(* Question 1.3: Multiplication *)
    ...
    let rec multiply a b = 
        ...
        | Num i1, Add (aa1, aa2)-> Add ((multiply (Num i1) aa1), (multiply (Num i1) aa2))
        | Add (a1, a2) , c ->  Add ((multiply a1 c), (multiply a2 c))
...
(* Question 3.1: Balanced brackets *)
    ...
    type wannabeStack = string List

    let emptyStack = [] : wannabeStack
    
    let balanced str = 
        let rec aux lst stac =
            match lst, stac with
            | [], [] -> true
            | [], stac -> false
            // | x::xs when x = "(" || x = ")" ->  
