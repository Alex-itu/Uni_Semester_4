...
(* Question 1.1 *)
    let rec countWhite img = 
        match img with
        | Square 255uy -> 1
        | Quad(fr,sc,th,fo) -> (countWhite fr) + (countWhite sc) + (countWhite th) + (countWhite fo)
        | Square _ -> 0
...
(* Question 1.2 *)
    let rotateRight img = 
        match img with
        | Square nr -> Square nr
        | Quad(fr, sc, th, fo) -> Quad(fo, fr, sc, th)

(* Question 1.3 *)
    let rec map mapper img =     
        match img with
        | Square nr ->  mapper nr
        | Quad(fr, sc, th, fo) -> Quad((map mapper fr), (map mapper sc), (map mapper th), (map mapper fo))
...
(* Question 1.4 *)
    ...
    let rec fold folder acc img = 
        match img with
        | Square nr ->  folder acc nr
        | Quad(fr, sc, th, fo) -> fold folder (fold folder (fold folder (fold folder acc fr) sc) th) fo
...
(* Question 4.2 *)
    let pushToStack nr stac : stack = 
        match stac with
        | [] -> [nr]
        | fullList -> nr :: fullList 

    let addFromStack stac =
        match stac with
        | [] -> failwith "empty stack"
        | _::[] -> failwith "empty stack"
        | x::xx::xs -> (x + xx) :: xs

    let multFromStack stac =
        match stac with
        | [] -> failwith "empty stack"
        | _::[] -> failwith "empty stack"
        | x::xx::xs -> (x * xx) :: xs
    ...
    let runStackProg prog = 
        let rec loop prog stac  =
            match prog with
            | [] -> List.head stac
            | (Push i)::xs -> loop xs (pushToStack i stac)
            | (Add)::xs -> loop xs (addFromStack stac)
            | (Mult)::xs -> loop xs (multFromStack stac)
        loop prog (emptyStack ()) 
